...
#trying to use that solution a bit.
#what I got from the stackoverflow question was that we could try to fully write our own merge function.
#That way, we could merge by id initially, and then write more code to merge by other sets of matches.
#I'm concerned that this is no easier than the initial merge function though, and that we should still 
#do that first. Then maybe we could write our own? But here's my attempt anyways.

#The first step, was to make a merged dataframe, which I create below. I made it look like what the merge function
# does.
mergedDF <- data.frame(corr_dob.x=new_tidy_arm_1$corr_dob, 
                       corr_mother.x=new_tidy_arm_1$corr_mother, 
                       corr_middle.x=new_tidy_arm_1$corr_middle,
                       gender.x=new_tidy_arm_1$gender,
                       grade.x=new_tidy_arm_1$grade,
                       ms_school.x=new_tidy_arm_1$ms_school,
                       hs_school.x=new_tidy_arm_1$hs_school,
                       id = new_tidy_arm_1$id,
                       matches=0)


#Then, here, I was trying to make a vector that is the true_id vector--has the id if it's in both arms, or
#NA if it's not. The idea was then I could add that as a column to mergedDF. But, the vector doesn't seem to work
#yet.
list <- vector('character')
  for (i in seq(new_tidy_arm_1$id)) {
    print(i)
    true_id = ifelse(new_tidy_arm_1$id[[i]] %in% new_tidy_arm_2$id, new_tidy_arm_1$id[[i]], NA)
    append(list, true_id)
    append(list, NA)
    print(true_id)
    print(list)
...
#Next thoughts: instead, write this for loop as a function, then call that function within a mutate.

#Other thoughts: maybe just go back to normal merging with the built-in function, merging by different columns, see how that goes...
#One question I have is what we were doing at the very end of last time--does merge by c(gender, school) merge 
#only if BOTH match, or if EITHER matches. If it's both, maybe we could just do a series of 
#standard merge function calls for various combinations of gender, school, dob, etc. that we think are likely
#matches.
